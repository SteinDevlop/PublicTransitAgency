...
@pytest.fixture(scope="module")  # Usa un fixture de pytest para la limpieza
def test_db():
...
def test_get_all_incidences(test_db):  # Inyecta el fixture
...
def test_get_incidence_by_id_existing(test_db):  # Inyecta el fixture
    ...
    incidence_id = created.IncidenciaID # Usa el atributo IncidenciaID
...
def test_get_incidence_by_id_not_found():
    ...
    assert response.status_code == 404 # Comprueba el código de estado 404
    assert response.json()["detail"] == "Incidence not found"  # Y el mensaje de error
