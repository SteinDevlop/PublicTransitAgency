...
class MockUniversalController:
    ...
    def get_by_id(self, model, id_):
        existing = self.movements.get(id_)
        if existing:
            return existing
        if not existing:
...
def test_create_movement():
    response = client.post("/typemovement/create", data={"id": 3, "type": "transfer"})
...
def test_update_existing_movement():
    response = client.post("/typemovement/update", data={"id": 1, "type": "investment"})
    assert response.status_code == 200
    data = response.json()
...
def test_update_nonexistent_movement():
    response = client.post("/typemovement/update", data={"id": 999, "type": "investment"})
    assert response.status_code == 404
    data = response.json()
    assert data["detail"] == "Movement type not found"

def test_delete_existing_movement():
    response = client.post("/typemovement/delete", data={"id": 1})
    assert response.status_code == 200
    data = response.json()
...
def test_delete_nonexistent_movement():
    response = client.post("/typemovement/delete", data={"id": 999})
    assert response.status_code == 404
    data = response.json()
    assert data["detail"] == "Movement type not found"
