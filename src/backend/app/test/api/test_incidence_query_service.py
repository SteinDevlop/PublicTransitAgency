...
from backend.app.api.routes.incidence_query_service import incidence_router
...
controller = UniversalController() # Inicializa el controlador aquí
...
def test_get_all_incidences(test_db):
    """Prueba que la ruta '/incidencias' devuelve correctamente todas las incidencias."""
    # Primero, crear algunas incidencias para probar
    incidence1_data = IncidenceCreate(Descripcion="Incidencia1", Tipo="Tipo1", TicketID=5)
    incidence2_data = IncidenceCreate(Descripcion="Incidencia2", Tipo="Tipo2", TicketID=6)
    controller.add(incidence1_data)
    controller.add(incidence2_data)
    response = client.get("/incidence/incidencias")
    assert response.status_code == 200
    data: List[Dict[str, Any]] = response.json()  # Type the data
    assert len(data) >= 2
    # Verificar que los datos devueltos coincidan con lo que se insertó
    assert data[0]["Descripcion"] in ["Incidencia1", "Incidencia2"]
    assert data[0]["Tipo"] in ["Tipo1", "Tipo2"]
    assert data[0]["TicketID"] in [5, 6]

def test_get_incidence_by_id_existing(test_db):
    ...
    incidence_data = IncidenceCreate(Descripcion="FindByIDE", Tipo="TipoIDE", TicketID=7)
    created = controller.add(incidence_data)
